...
class UserController {
    ...
    static async registerSuperAdmin(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const { firstName, lastName, email, phone, password, setupKey } = req.body;
            ...
            const token = UserController.generateToken(superAdmin._id.toString(), superAdmin.role);
    ...
    static async checkSuperAdminExists(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
    ...
    static async register(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            if (["super_admin", "admin"].includes(role)) {
                throw new AppError("Cannot register super admin or admin through this endpoint", 400);
            }
            ...
            const token = UserController.generateToken(user._id.toString(), user.role);
    ...
    static async forcePasswordChange(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const { newPassword, tempToken } = req.body;

            if (!newPassword || !tempToken) {
                throw new AppError("New password and temporary token are required", 400);
            }
